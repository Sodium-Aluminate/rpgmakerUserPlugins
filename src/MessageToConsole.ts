...
(() => {
	...
	switch (Utils?.RPGMAKER_NAME) {
		...
		case "MZ":
			...
			injector.inject("Window_BattleLog.prototype.addText", "HEAD", function (cir) {
				const [text] = cir.args as [string];
				console.log(text)
			})
			break
		default:
	...
	// https://github.com/munokura/tomoaky-MV-plugins/blob/master/TMLogWindow.js
	if (PluginManager.parameters("TMLogWindow"))
		injector.inject("Game_System.prototype.addLog", "HEAD", function (cir) {
